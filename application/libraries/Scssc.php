...
class Scssc {
	...
	protected function importFile($path, $out) {
		...
		if (isset($this->importCache[$realPath])) {
            ...
            } else if(filemtime($path) > $this->last_modified) {
                $this->last_modified = filemtime($path);
            }
...
class scss_server {
	...
	protected function needsCompile($in, $out) {
		...
		$mtime = filemtime($out);
        $this->scss->last_modified = filemtime($in);
        $this->scss->compile(file_get_contents($in), $in);
        //$mtime = $this->scss->last_modified;

		if ($this->scss->last_modified > $mtime) return true;
	 ...
	 */
	public function serve($salt = '', $name = '') {
